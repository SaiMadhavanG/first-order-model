...
class FramesDataset(Dataset):
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
                 random_seed=0, pairs_list=None, augmentation_params=None, deterministic=False, lmks_available=True):
        ...
        self.lmks_available = lmks_available

        self.root_dir = os.path.join(root_dir, 'images')
        self.lmks_dir = os.path.join(root_dir, 'lmks')
        ...
        if os.path.exists(os.path.join(self.root_dir, 'train')):
            ...
            self.lmks_dir = os.path.join(self.lmks_dir, 'train' if is_train else 'test')
        else:
            raise('Error Occurred! Pls check data directory format.')
    ...
    def __getitem__(self, idx):
        ...
        lmks_path = os.path.join(self.lmks_dir, name)

        video_name = os.path.basename(path)
        audio_path = os.path.join(self.audio_dir, video_name + '.npy')

        video_array = read_video(path, frame_shape=self.frame_shape)
        lmks_array = read_video(lmks_path, frame_shape=self.frame_shape)
        ...
        lmks_array = lmks_array[frame_idx]
        audio_array = audio_array[frame_idx]
